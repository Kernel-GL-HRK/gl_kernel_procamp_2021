				  ...
				  struct kobj_attribute *attr, char *buf)
{
	...
	if (unlikely(!has_read_request)) {
		...
		bytes_show =
			sprintf(buf, "Relative cycles count:%lld \n", ellapsed);
		update_use_request();
	}
	return bytes_show;
}
				  ...
				  struct kobj_attribute *attr, char *buf)
{
	...
	if (unlikely(!has_read_request)) {
		...
		bytes_show = sprintf(
			buf, "Last time absolute read: %d:%d:%d %d/%d/%ld\n",
			current_time.tm_hour, current_time.tm_min,
			current_time.tm_sec, current_time.tm_mday,
...
struct kobj_attribute relative_time_attr =
	__ATTR(rw_node, 0660, relative_time_show, NULL);
struct kobj_attribute absolute_time_attr =
	__ATTR(absolute_time_node, 0660, absolute_time_show, NULL);

static struct attribute *timer_attributes[] = { &relative_time_attr.attr,
						&absolute_time_attr.attr,
						NULL };

static struct attribute_group timer_group = { .attrs = timer_attributes };

static __init int time_module_init(void)
{
	int result = 0;
	timer_mod_entry_root =
		kobject_create_and_add("timer_module_dir", kernel_kobj);
...
MODULE_DESCRIPTION(
	"A module for obtaining absolute time and last sysfs entry reading relative time");