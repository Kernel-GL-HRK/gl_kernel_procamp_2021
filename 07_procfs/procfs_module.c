...
#define MODULE_AUTHOR_STRING "Valentyn Korniienko <valentyn.korniienko1@nure.ua>"
...
static int __init simple_struct_module_init(void)
{
    ...
    if (!proc_create_single("module_author", S_IWUSR |S_IRUSR | S_IRGRP | S_IROTH, proc_entry_module_root, procfs_module_show_author)){
		pr_warn("unable to register /proc/task07_module_entries/task07_module_author\n");
        goto cleanup_kobject_and_procfs;
    }
    if (!proc_create_single("write_requests", S_IWUSR |S_IRUSR | S_IRGRP | S_IROTH, proc_entry_module_root, procfs_module_show_write_requests)){
    ...
    if (!proc_create_single("read_requests", S_IWUSR |S_IRUSR | S_IRGRP | S_IROTH, proc_entry_module_root, procfs_module_show_read_requests)){
