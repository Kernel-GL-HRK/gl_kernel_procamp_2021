 ...
 *
 * and modified to return only index.html page
 */
...
int main(int argc, char **argv)
{
	...
	/* variable for connction with chardev */
	int fdev;
...
//							);
		...
		printf("%s", buf);
		if (sscanf(buf, "%s %s %s\n", method, uri, version) == EOF)
			error("ERROR on sscanf");
		...
		if (!strstr(uri, "cgi-bin")) {
			/* static content */
			printf(" static content: %s\n", uri);
			if (strstr(uri, "red=on") != NULL) {
				/* static content */
				...
				printf("!!red=on\n");
			} else if (strstr(uri, "red=off") != NULL) {
				/* static content */
				fdev = open(DEVPATH, O_RDWR);
				if (fdev >= 0) {
				...
				printf("!!red=off\n");
			} else if (strstr(uri, "green=on") != NULL) {
				/* static content */
				fdev = open(DEVPATH, O_RDWR);
				if (fdev >= 0) {
				...
				printf("!!!green=on\n");
			} else if (strstr(uri, "green=off") != NULL) {
				/* static content */
				fdev = open(DEVPATH, O_RDWR);
				if (fdev >= 0) {
		...
		if (is_static) {
			...
			if (!(S_IFREG & sbuf.st_mode) || !(/*S_IXUSR*/ 0100 & sbuf.st_mode)) {
