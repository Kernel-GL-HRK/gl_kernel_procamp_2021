...
const char *alloc_function_name[] = { "kmalloc", "kvmalloc", "get_free_pages",
				      "kzalloc" };
...
static void profile_alloc(alloc_profile_mode_t profile_mode)
{
	...
	do {
		...
		pr_debug(
			"Allocation:%s, Bytes:%ld, order: %ld, cycles for request: %lld\n",
			alloc_function_name[profile_mode], alloc_request, order,
		...
		pr_debug(
			"Freeing:%s, Bytes:%ld, order:%ld, cycles for request: %lld\n",
			alloc_function_name[profile_mode], alloc_request, order,
			(freeing_end - freeing_start));
