...
filter() {
    # tail + grep combo return a single block of strings separated with
    # spaces instead of '\n's. udevadm does not have an ability to filter
    # events by action types by itself so, for example inserting a new
    # USB device generates lots of output which contains also 'bind' actions
    # wehich we consider just a noise for this task. These must be excluded
    # from the output, so we iterate through the list of words in search
    # for the give tag (like 'add' or 'remove'); in case one is found
    # the next token is device name (according to udevadm format). Sub-classes
